...
- 你对着朋友 / 伴侣讲过自己的童年 / 职业 / 纠结,但想要一个能**持续对话、不会陪你演戏**的第三方
- 你发现某个坑每次都掉进去又掉出来,但说不清是什么
- 你对 AI 陪聊有疑虑 —— 因为它**太会附和你**,你想要的是**反面**

### 你会从它拿到的

| 能力 | 具体表现 |
|---|---|
| **长期记忆** | 半年前你说过的话它记得;今天你说同一件事,它会把两次叙述放一起对照 |
| **Pattern 识别** | 你以为是 3 次独立事件,它会告诉你是**同一个机制**的 3 次上演 |
| **Narrative vs 现实校准** | 你说"我不敢 X",它拿你**实际做过**的 Y 来对照(`/compile-me` 命令) |
| **反附和** | 你说"我只是运气不好",它会追问"为什么同样的运气在别人身上不出现" |
| **结构化档案** | 不是 chat history,是你**可以读 / 改 / 引用 / 分享**的文件 |

### 不要指望它

- **替代心理咨询** — 它不是治疗师,遇到临床红线会建议你找专业人士
- **第一次 session 就深度剖析** — 深度是累积出来的,第 5 次 session 之后才开始产出真货
- **取悦你** — 这是它最大的 anti-feature
